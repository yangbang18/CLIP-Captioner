...
class Translator_NARFormer(object):
    ...
    def translate_batch(self, models, batch, teacher_model_wrapper=None, vocab=None):
        ...
        if teacher_model_wrapper is not None:
            dict_mapping = get_dict_mapping(self.opt, teacher_model_wrapper.get_opt())
        ...
        # prepare tgt_tokens
        preds_length = encoding_phase_outputs['preds_length']
        beam = self.predict_length_beam(preds_length, self.length_beam_size, self.length_bias)   
        max_len = beam.max().item()

        length_mask = torch.triu(preds_length.new(max_len, max_len).fill_(1).long(), 1)
        length_mask = torch.stack([length_mask[beam[batch] - 1] for batch in range(bsz)], dim=0)

        tgt_tokens = preds_length.new(bsz, self.length_beam_size, max_len).fill_(Constants.MASK).long()
        ...
        # run the algorithm
        algorithm = self.algorithms_mapping[self.paradigm](self.opt, dict_mapping, vocab)
    ...
    def predict_length_beam(self, predicted_lengths, length_beam_size, length_bias):
...
def get_dict_mapping(opt, teacher_opt):
    ...
    teacher_info = pickle.load(open(teacher_opt["info_corpus"], 'rb'))['info']
    teacher_vocab = teacher_info['itow']
