...
def get_criterion(opt, skip_crit_list=[]):
    assert isinstance(opt['crits'], list)
    satisfied_crits = [item for item in opt['crits'] if item not in skip_crit_list]

    crit_objects = []
    names = []
    scales = []
    for crit in satisfied_crits:
        info = get_crit_info(opt, crit)
