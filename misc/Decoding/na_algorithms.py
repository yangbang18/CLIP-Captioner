...
class Algorithm_Base(object):
    ...
    def __init__(self, opt, dict_mapping, vocab):
    ...
    def get_coarse_grained_templates(self, model, inputs_for_decoder, tgt_tokens, pad_mask):
        ...
        tgt_tokens, token_probs = self.generate_non_autoregressive(model, inputs_for_decoder, tgt_tokens, pad_mask)
        token_probs[tgt_tokens.eq(Constants.MASK)] = 0.0
        return tgt_tokens, token_probs

    def generate_non_autoregressive(self, model, inputs_for_decoder, tgt_tokens, pad_mask, zeros=[]):
...
class MaskPredict(Algorithm_Base):
    def __init__(self, opt, dict_mapping, vocab):
        super().__init__(opt, dict_mapping, vocab)
